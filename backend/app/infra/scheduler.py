...
def youth_policy_crawling_batch():
    logger.info("youth_policy_crawling_batch start")
    save_policy_crawling_chunks()
    logger.info("youth_policy_crawling_batch end")
...
def youth_policy_crawling_batch_job():
    logger.info("youth_policy_crawling_batch_job start")
    youth_policy_crawling_batch()
    logger.info("youth_policy_crawling_batch_job end")
...
def start_scheduler():
    ...
    scheduler.add_job(
        youth_policy_crawling_batch_job,
        trigger="cron",
        hour="00",
        minute="20",
        id="youth_policy_crawling_batch_job",
        replace_existing=True,  # id가 겹칠시 덮어쓰기 할 지 여부 설정
        max_instances=1,        # 하나의 job 이 동시에 몇 개까지 실행될 수 있는지 설정
        coalesce=True,          # 밀린 실행들을 각각 실행할지(False), 하나로 합쳐 실행할지(True) 설정
        misfire_grace_time=1200,  # 스케줄 시간이 초과했을 때, 늦게 실행을 허용해주는 시간 설정
